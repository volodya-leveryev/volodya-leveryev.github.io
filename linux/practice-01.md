...
2. Проверьте доступность сервера, используя команду `ping`. Учтите, что отсутствие ответа на ping не обязательно означает, что сервер недоступен по SSH.

   ```bash
   ping <адрес сервера>
   ```

3. Перейдите в каталог, в который скачали файл с приватным ключом. По умолчанию терминал открывается в домашнем каталоге пользователя, обычно каталог для скачивания называется `Downloads`:
...
4. Убедитесь, что файл с приватным ключом присутствует в текущем каталоге (в ряде случаев вместо `ls` используется команда `dir`):
...
9. Откройте новый терминал на **локальном компьютере** и сгенерируйте пару ключей:
...
10. Выведите содержимое публичного ключа в терминал:
...
11. На сервере откройте файл `~/.ssh/authorized_keys` в текстовом редакторе, например `nano`:
...
12. Подключитесь к серверу под именем вновь созданного пользователя с помощью приватного ключа (составьте команду и её аргументы самостоятельно). Если подключение выполнено успешно, в приглашении командной строки должно отображаться имя созданного пользователя.
...
Установите на локальный компьютер программу [Filezilla](https://filezilla-project.org/download.php?type=client) и попробуйте подключиться к серверу по SSH (порт 22) под новым пользователем, которого вы создали. Проверьте, как происходит копирование файлов в обеих направлениях с помощью этой программы.
