...
12. Откройте сервис `CloudFront` создайте новый Distribution:
    ...
    * Выбрать `Pay as you go`
    * Внести имя дистрибьюшена в формате <группа>-<фамилия>
    * Выбрать cозданный ранее Amazon S3 бакет в качестве Origin
    * Нажать кнопку `Use website endpoint` после выбора бакета
    * В разделе Web Application Firewall (WAF) выберите "Do not enable…"

    После создания скопируйте доменное имя Distribution.

14. Откройте сервис Route53, раздел Hosted zones и зону `kit-imi.info`. Cоздайте доменное имя типа CNAME которое указывает на доменное имя вашего Cloudfront Distribution (без `https://`). В качестве доменного имени впишите `<имя группы>-<фамилия студента>`.

15. Вернитесь в сервис `CloudFront`, откройте созданный Distribution, на вкладке `General` нажмите кнопку `Add domain`:

    * В качестве домена выберите созданное в Route 53 в формате `<имя группы>-<фамилия студента>.kit-imi.info`
    * Выберите SSL-сертификат `*.kit-imi.info`

17. Откройте `Git Bash` и выполните команду:

    ```
    curl.exe -v <URL файла clip.webm с S3> -o clip.webm
    ```
...
16. Откройте новое окно `Git Bash`. Несколько раз выполните команду (используйте `http`-вариант ссылки)
