...
<pre><code class="language-python"> 
    ...
    try: 
       ...
       print(f"Не удалось удалить: {e}") </code></pre>
...
# Удаление переменной
del os.environ['MY_APP_MODE'] </code></pre>
...
print("Конфигурация:", config)</code></pre>
...
<pre><code class="language-python"> import os 
    ...
    os.environ['MY_VAR'] = 'value' # Предпочтительный способ </code></pre>
...
if result != 0:
    print("Команда завершилась с ошибкой") </code></pre>
...
print(f"Python установлен: {программа_установлена('python')}") </code></pre>
...
if os.name == 'nt':  # Проверяем что это Windows
    ...
    print("os.startfile() доступен только в Windows") </code></pre>
...
if os.name == 'nt':
    ...
    print("Неизвестная ОС") </code></pre>
os.sep и os.altsep — разделители путей
<pre><code class="language-python"> import os

print(f"Основной разделитель путей: '{os.sep}'")
print(f"Альтернативный разделитель: '{os.altsep}'") </code></pre>
...
if cpu_count:
    ...
    print(f"Рекомендуемое количество потоков: {workers}") </code></pre>
...
print(f"Случайный токен: {token}") </code></pre>
...
ppid = os.getppid()
print(f"ID родительского процесса: {ppid}") </code></pre>
### Практические задачи
...
# мониторинг_директории('.', интервал=2) </code></pre>
...
# Пример использования
дубликаты = поиск_дубликатов_файлов(".") </code></pre>
