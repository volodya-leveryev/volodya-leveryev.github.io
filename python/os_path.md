# Конспект по модулю os.path в Python
## Содержание
1 Введение

2 Объединение и разбор путей

3 Абсолютные и относительные пути

4 Проверка существования файлов и папок

5 Разделение пути на компоненты

6 Получение информации о файле

7 Время файлов

8 Примеры комплексного использования
...
print(f"Модуль os.path доступен: {hasattr(os, 'path')}")</code></pre>
### 2. Объединение и разбор путей
*os.path.join(path, paths) — объединяет части пути
Создает корректные пути, автоматически добавляя правильные разделители для текущей ОС.
...
print(f"Путь с '..': {result}")</code></pre>
os.path.normpath(path) — нормализует путь
Убирает лишние слэши и обрабатывает . (текущая директория) и .. (родительская директория).
...
user_path = "C:\\Users\\\\John\\Documents\\..\\Desktop\\file.txt"</code></pre>
clean_path = os.path.normpath(user_path)
print(f"Очищенный путь: {clean_path}")</code></pre>
os.path.commonpath(paths) — общий путь для списка путей
Находит самый длинный общий путь для нескольких путей.
...
print(f"Общий путь (Windows): {common_win}")</code></pre>
### 3. Абсолютные и относительные пути
os.path.abspath(path) — абсолютный путь
Возвращает полный путь от корня файловой системы.
...
print(f"Абсолютный: {absolute}")</code></pre>
os.path.relpath(path, start='.') — относительный путь
Возвращает путь от start до path.
...
print(f"Переносимый путь: {portable}")</code></pre>
os.path.isabs(path) — проверка абсолютного пути
...
for path in paths_to_check:
    ...
    print(f"{path:30} -> Абсолютный: {is_absolute}")</code></pre>
### 4. Проверка существования файлов и папок
...
for path in paths_to_check:
    ...
    print(f"  {path:20} {status}")</code></pre>
...
print(f"\nФайлы в текущей директории: {files}")</code></pre>
os.path.isdir(path) — является ли папкой
...
print(f"\nВсего найдено директорий: {len(all_dirs)}")</code></pre>
os.path.islink(path) — является ли символической ссылкой
Проверяет, является ли путь символической ссылкой (актуально для Unix-like систем).
...
if os.name != 'nt':  # Проверяем, что не Windows
    ...
    print("Символические ссылки не поддерживаются в Windows через эту функцию")</code></pre>
os.path.ismount(path) — является ли точкой монтирования
...
for path in paths_to_check:
    if os.path.exists(path):  # Проверяем существование перед проверкой
        ...
        print(f"  {path:20} -> {status}")</code></pre>
### 5. Разделение пути на компоненты
os.path.split(path) — разделяет путь на (директория, файл)
Разделяет путь на кортеж (head, tail), где tail - последний компонент пути.
...
print(f"Директория на 2 уровня выше: {parent}")</code></pre>
os.path.splitext(path) — разделяет имя файла и расширение
Разделяет путь на кортеж (root, ext), где ext начинается с точки.
...
for test in test_cases:
    ...
    print(f"  {test:25} -> Имя: '{name}', Расширение: '{ext}'")</code></pre>
os.path.basename(path) — имя файла или последний компонент пути
Возвращает последний компонент пути (аналог os.path.split()[1]).
...
process_file("/var/log/app/app.log")</code></pre>
os.path.dirname(path) — директория пути
Возвращает директорию пути (аналог os.path.split()[0]).
...
print(f"Созданный путь: {full_path}")</code></pre>
os.path.commonprefix(list) — общий префикс путей
Устаревшая функция (лучше использовать os.path.commonpath()).
...
print(f"Общий префикс: {common_prefix}")</code></pre>
### 6. Получение информации о файле
...
for size in test_sizes:
    ...
    print(f"{size:15} байт = {readable}")</code></pre>
os.path.getatime(path) — время последнего доступа
Возвращает время последнего доступа к файлу в секундах с начала эпохи.
...
if os.path.exists("data.txt"):
    ...
    print(f"Дней с последнего доступа: {days_since_access:.1f}")</code></pre>
os.path.getmtime(path) — время последней модификации
...
if os.path.exists("data.txt"):
    ...
    if newest:
        print(f"Самый новый файл: {newest}")</code></pre>
os.path.getctime(path) — время создания файла
Возвращает время создания файла (на Unix-системах - время изменения метаданных).
...
if os.path.exists("data.txt"):
    ...
    if create_time != modify_time:
        print("Файл был изменен после создания")</code></pre>
### 7. Время файлов
os.path.samefile(path1, path2) — один и тот же файл?
Проверяет, ссылаются ли пути на один и тот же файл (учитывает символические ссылки).
...
os.remove("different.txt")</code></pre>
os.path.samestat(stat1, stat2) — одинаковая stat информация?
Проверяет, ссылаются ли две stat структуры на один и тот же файл.
...
if os.path.exists("test.txt"):
    ...
    print(f"Stat структуры указывают на один файл: {same}")</code></pre>
### 8. Примеры комплексного использования
...
# Использование
analyze_logs_directory()</code></pre>
...
# Использование
create_file_system_report()</code></pre>
...
# Использование
analyze_directory_size()</code></pre>
